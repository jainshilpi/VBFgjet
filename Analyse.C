...
void Analyse::Loop(string fname, string fnameout)
{
  ...
  hmap["ratio_trackp_To_Jetp"] = new TH1F("ratio_trackp_To_Jetp","",500,-10,10);
  ...
  for(map<string,TH1F*>::iterator it = hmap.begin(); it != hmap.end(); ++it) {
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	eta_pho_jetSys_norm = ( Photon_Eta[phoInd] - (jet1eta+jet2eta)/2. )/((jet1eta+jet2eta)/2.);
	...
	cout<<"Constitutes of jets "<<Jet_Constituents[jet1Ind].GetEntries()<<endl;
	
	for(int jc = 0; jc < Jet_Constituents[jet1Ind].GetEntriesFast(); ++jc)
	  {
	    TObject *object = Jet_Constituents[jet1Ind].At(jc);
	...
	hmap["ratio_trackp_To_Jetp"]->Fill(ratio_trackp_To_Jetp);
